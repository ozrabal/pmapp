...
export class AIService {
  ...
  async generateText(input: TextGenerationInput): Promise<AIServiceResponse<TextGenerationResponse>> {
    try {
      ...
      const messages = [];
      if (validatedInput.system) {
        messages.push({ role: "system" as const, content: validatedInput.system });
      }
      if (validatedInput.prompt) {
        messages.push({ role: "user" as const, content: validatedInput.prompt });
      }
      if (validatedInput.messages) {
        messages.push(...validatedInput.messages);
      }
