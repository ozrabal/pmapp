CREATE TYPE IF NOT EXISTS "public"."estimation_unit_enum" AS ENUM('hours', 'storypoints');--> statement-breakpoint
CREATE TYPE IF NOT EXISTS "public"."task_dependency_type_enum" AS ENUM('finish_to_start', 'start_to_start', 'finish_to_finish', 'start_to_finish');--> statement-breakpoint
CREATE TYPE IF NOT EXISTS "public"."task_priority_enum" AS ENUM('low', 'medium', 'high');--> statement-breakpoint
...
ALTER TABLE "chat_sessions" ADD CONSTRAINT "chat_sessions_user_id_fkey" FOREIGN KEY ("user_id") REFERENCES "auth"."users"("id") ON DELETE no action ON UPDATE no action;--> statement-breakpoint
...
CREATE INDEX "idx_chat_messages_session_time" ON "chat_messages" USING btree ("session_id","timestamp" DESC NULLS LAST);--> statement-breakpoint
CREATE INDEX "chat_messages_content_text_gin" ON "chat_messages" USING gin (to_tsvector('english', coalesce("content_text", '')));--> statement-breakpoint
CREATE INDEX "idx_chat_sessions_user_updated" ON "chat_sessions" USING btree ("user_id","updated_at" DESC NULLS LAST);--> statement-breakpoint
